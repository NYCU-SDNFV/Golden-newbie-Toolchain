...
def validate_metadata(data, *, allow_development=False):
    ...
    if data["template"] != f"NYCU-SDNFV/Golden-{channel}-Toolchain":
...
def check_freshness(repo):
    ...
    if version_tuple(current["version"]) < version_tuple(latest["version"]):
        ...
        print(f"Lab0 {current['version']} is current ({current['channel']}).")
    return True
...
def update(repo):
    ...
    if latest["tag"] == current["tag"]:
        print(f"Lab0 {current['version']} is already current; no branch or commit created.")
        return
    ...
    try:
        _git(repo, "commit", "-m", f"Update Lab0 to {latest['tag']}\n\n{COAUTHOR}")
...
def main(argv=None):
    ...
    try:
        ...
        print(f"Lab0: {exc}", file=sys.stderr)
        return 1
    return 0
